...
def spin_lobby(current_user, start_game_func):
    ...
    while running:
        screen.fill((0,0,0))
        header = header_font.render("Spin Mode", True, (255,255,255))
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for d, rect in difficulty_rects:
                    if rect.collidepoint(mx,my):
                        print(f"Spin started for {current_user} at {d}")
                        t0 = start_timer()
                        # Call actual game function
                        # start_game_func(current_user, mode="Spin", difficulty=d)
                        survive_time = end_timer(t0)
                        xp, gold = add_rewards(current_user,"Spin",d,survive_time,accuracy=100)
                        update_leaderboard(current_user,"Spin",survive_time)
